...
export default function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6 no-print">
        <Link href={`/admin/reservations/${id}`} className="text-gray-400 hover:text-gold text-lg flex-shrink-0">
          ←
        </Link>
        <div className="flex-1 min-w-0">
          <h1 className="text-lg sm:text-2xl font-bold text-gray-900">Invoice</h1>
          <p className="text-[11px] sm:text-sm text-gray-400 font-mono truncate">{invoice.invoiceNumber}</p>
        </div>
        <button
          onClick={handlePrint}
          className="px-3 sm:px-6 py-2.5 sm:py-3 bg-gold text-white font-semibold rounded-xl hover:bg-gold-dark transition-all text-xs sm:text-sm whitespace-nowrap active:scale-[0.98]"
        >
          🖨️ Cetak
        </button>
      </div>
